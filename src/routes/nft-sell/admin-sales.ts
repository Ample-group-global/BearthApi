...
import { requireRegisteredWallets, triggerChainSync } from "../../services/customer-whitelist.service";
...
router.post("/", requireAdmin, async (req, res, next) => {
  try {
    ...
    // Strict: buyer wallet must be pre-registered as a customer.
    await requireRegisteredWallets([buyerAddress]);
...
router.post("/:id/mint", requireAdmin, async (req, res, next) => {
  try {
    ...
    await requireRegisteredWallets([sale.buyer_address as string]);
    triggerChainSync();
